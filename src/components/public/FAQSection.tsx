    ...
    <section id="faq" className="py-20 bg-slate-50 dark:bg-slate-900/60 border-t border-slate-200 dark:border-slate-800 transition-colors">
      <div className="max-w-5xl mx-auto px-4 sm:px-6">
        ...
        <div className="text-center max-w-2xl mx-auto mb-14 space-y-3">
          <span className="text-xs font-bold uppercase tracking-widest text-amber-600 dark:text-amber-400 bg-amber-500/10 border border-amber-500/20 px-3.5 py-1 rounded-full inline-flex items-center gap-1.5">
            <EditableIcon iconKey="faq.badgeIcon" defaultIcon="HelpCircle" defaultColor="#f59e0b" size={14} />
            <span><EditableText blockKey="faq.badge" defaultText="Frequently Asked Questions" /></span>
          </span>
          
          <h2 className="text-3xl sm:text-4xl font-serif font-black text-slate-900 dark:text-white tracking-tight">
            <EditableText blockKey="faq.heading" defaultText="Frequently Asked Questions" />
          </h2>
          
          <p className="text-sm text-slate-600 dark:text-slate-400 font-body">
            <EditableText blockKey="faq.subtext" defaultText="Common questions regarding admissions, curriculum, transport, and school policies." />
          </p>
        </div>
              ...
              >
                ...
                >
                  <div className="space-y-1">
                    <span className="text-[11px] font-extrabold text-amber-600 dark:text-amber-400 uppercase tracking-wider">
                      <EditableText blockKey={`faq.${faq.id}.category`} defaultText={faq.category} />
                    </span>
                    <h3 className="text-base sm:text-lg font-bold text-slate-900 dark:text-white">
                      <EditableText blockKey={`faq.${faq.id}.question`} defaultText={faq.question} />
                    </h3>
                  </div>
                  <div
                ...
                {isOpen && (
                  <div className="px-5 sm:px-6 pb-6 pt-1 text-xs sm:text-sm text-slate-600 dark:text-slate-300 leading-relaxed border-t border-slate-100 dark:border-slate-700/50">
                    <p>
                      <EditableText blockKey={`faq.${faq.id}.answer`} defaultText={faq.answer} />
                    </p>
                  </div>
                )}
              </div>
        ...
        <div className="mt-12 p-6 sm:p-8 rounded-3xl bg-gradient-to-br from-amber-500/10 via-brand-primary/5 to-slate-900/5 dark:from-slate-800 dark:to-slate-850 border border-amber-500/30 flex flex-col md:flex-row items-center justify-between gap-6 shadow-sm">
          <div className="space-y-2 text-center md:text-left">
            <h4 className="text-lg sm:text-xl font-bold text-slate-900 dark:text-white flex items-center justify-center md:justify-start gap-2">
              <EditableIcon iconKey="faq.ctaSchoolIcon" defaultIcon="School" defaultColor="#f59e0b" size={20} />
              <span><EditableText blockKey="faq.ctaQuestion" defaultText="Have more questions about admissions or campus visits?" /></span>
            </h4>
            <p className="text-xs sm:text-sm text-slate-600 dark:text-slate-400">
              <EditableText blockKey="faq.ctaDesc" defaultText="Speak directly with our admission counselors or visit our campus at Bhawanipur, Sikta." />
            </p>
          </div>
          ...
          <div className="flex flex-wrap items-center justify-center gap-3">
            ...
            >
              <EditableIcon iconKey="faq.phoneIcon" defaultIcon="Phone" defaultColor="#f59e0b" size={16} />
              <span><EditableText blockKey="phones" defaultText={settings?.phones?.split(',')[0] || '+91 87579 68130'} /></span>
            </a>
            <a
              href="#admissions"
            ...
            >
              <EditableIcon iconKey="faq.gradIcon" defaultIcon="GraduationCap" defaultColor="#020617" size={16} />
              <span><EditableText blockKey="faq.ctaApplyBtn" defaultText="Apply for Admission" /></span>
            </a>
          </div>
        </div>
